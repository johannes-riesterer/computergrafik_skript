...
Das sogenannte photometrische Grundgesetzt beschreibt die Strahlung, die von einem infintesimalen Flächenelement auf ein anderes infinitesimales Flächenelement 
übertragen wird.  
 \begin{figure}[H]
    \centering
...
\end{align}
Hierbei ist $L$   die Radiance oder auch Stahldichte  welche durch  
\begin{align}
L(x, \omega) := \frac{d^2 \phi}{\cos(\theta) \cdot dA \cdot d\omega} 
\end{align}
definiert werden kann, wobei $\theta$ der Winkel zwischen der Flächennormale am Punkt $x$ und der Richtung $\omega$ ist.
