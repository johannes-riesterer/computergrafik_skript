...
In einer Liste $E = (e_1,~e_2,~\hdots,~e_n)$ werden Referenzen auf die Ecken 
gespeichert. 
In einer Liste $K$ werden die Kanten als Tupel von Referenzen auf die Eckenliste 
abgespeichert: 
\begin{equation*}
    K = \bigl( (i_{e_1},~\hdots,~i_{e_n}),~\hdots,~
               (i_{e_1},~\hdots,~i_{e_m}) \bigr)
\end{equation*}
Die $k$-te Facette wird als Liste von Referenzen auf die Kantenliste 
gespeichert: 
\begin{equation*}
    F_k = \bigl( K[j_1],~\hdots,~K[j_l] \bigr)
\end{equation*}
In einer Liste $M$ werden die Referenzen auf Facetten in Tupeln abgespeichert, 
...
\begin{itemize}
    \item Referenzen auf den Anfangs- und Endpunkt ($i_b$ und $i_e$)
